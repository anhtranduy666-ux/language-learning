/**
 * Mọi âm tiết có thật của tiếng Phổ thông, viết như pinyin chuẩn và bỏ dấu
 * thanh — 413 âm tiết. `ü` viết là `v` (`nv`, `lve`), đúng dạng mà
 * `src/lib/speechTokens.ts` đưa cho máy đọc; sau j/q/x/y thì vẫn là `u`.
 *
 * Chỉ test dùng bảng này, để bắt pinyin gõ nhầm trong dữ liệu (`shie`,
 * `zhogn`) trước khi máy đọc cố phát âm một thứ không tồn tại.
 */
export const PINYIN_SYLLABLES: ReadonlySet<string> = new Set(
  ...
  cang cao ce cei cen ceng cha chai chan chang chao che chen cheng chi chong chou chu chua
  chuai chuan chuang chui chun chuo ci cong cou cu cuan cui cun cuo da dai dan dang dao de
  dei den deng di dia dian diao die ding diu dong dou du duan dui dun duo e ei en eng er fa
  fan fang fei fen feng fo fou fu ga gai gan gang gao ge gei gen geng gong gou gu gua guai
  ...
  kao ke kei ken keng kong kou ku kua kuai kuan kuang kui kun kuo la lai lan lang lao le lei
  leng li lia lian liang liao lie lin ling liu lo long lou lu luan lun luo lv lve ma mai man
  mang mao me mei men meng mi mian miao mie min ming miu mo mou mu na nai nan nang nao ne
  nei nen neng ni nian niang niao nie nin ning niu nong nou nu nuan nun nuo nv nve o ou pa
