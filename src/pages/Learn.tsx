  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{HSK1.title}</h1>
          <p className="text-sm text-slate-500">{HSK1.description}</p>
        </div>
        <StatBar />
      </header>
      ...
      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
        <div className="flex items-baseline justify-between">
          <h2 className="font-semibold text-slate-900">Tiến độ khoá học</h2>
      ...
      {HSK1.units.map((unit) => (
        <section key={unit.id} className="space-y-3">
          <div>
            <h2 className="font-semibold text-slate-900">{unit.title}</h2>
              ...
              return (
                <li key={lesson.id}>
                  <Link
                    to={`/lesson/${lesson.id}`}
                    className={cn(
                      'flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm transition hover:bg-slate-50',
                      isNext ? 'ring-2 ring-brand-400' : 'ring-1 ring-slate-100',
                    )}
                  >
                    <span
                      aria-hidden="true"
                      className={cn(
                        ...
                        isDone ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400',
                      )}
                    >
                      {isDone ? '✓' : '📇'}
                    </span>
